...
CREATE TABLE `articale` (
  ...
  `user_id` int(11) NOT NULL COMMENT '文章属于的前台用户id,后台填写是后台用户id',
  `sys_user_id` int(5) NOT NULL DEFAULT '0' COMMENT '文章所属的后台用户id,前台用户默认是0',
  ...
  `commentSum` int(5) NOT NULL DEFAULT '0' COMMENT '文章评论数量,用户评论后由消息队列进行更新',
  `browseSum` int(10) NOT NULL DEFAULT '0' COMMENT '文章访问数量,查看文章后由消息队列进行更新',
...
CREATE TABLE `comment` (
  ...
  `category_id` int(11) NOT NULL COMMENT '评论分类id',
  `articale_id` int(11) NOT NULL COMMENT '评论文章id',
  `parent_id` int(11) NOT NULL DEFAULT '0' COMMENT '父评论id,是否评论嵌套',
  ...
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='评论的数据表,添加parent_id和root_id字段使其支持评论嵌套';
...
CREATE TABLE `user` (
  ...
  `articale_sum` int(100) NOT NULL DEFAULT '0' COMMENT '用户的文章数量,添加文章后由消息队列进行更新',
...
CREATE TABLE `user_dynamic` (
  `id` int(10) unsigned NOT NULL AUTO_INCREMENT COMMENT '动态主键id',
  `type` int(5) NOT NULL DEFAULT '0' COMMENT '动态类型.1:文章相关,2:评论相关',
  `action` int(11) NOT NULL DEFAULT '0' COMMENT '动态动作.1:回复',
  `user_id` int(11) NOT NULL COMMENT '动态发起者的id',
  `target_id` int(11) NOT NULL COMMENT '动态类型的id,可能是文章或者是评论,取决于type字段',
  `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建动态的时间',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='动态数据表.用于监视用户的动作';
...
CREATE TABLE `user_follow` (
  ...
  `followStatus` int(10) NOT NULL DEFAULT '0' COMMENT '关注的状态.1:关注,2:不关注',
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='用户关注关系的数据表';
